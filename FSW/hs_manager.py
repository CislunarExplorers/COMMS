...
class hs_manage:
    ...
    gvHS_myApid = 3 #change later
    gvHS_maxCmdQ = 50 #max commands waiting

    #gvHS_command = ccsds_header.CCSDS_CreateCommand
    gvHS_command = ccsds_header.CCSDSCommand()
    
    def health_test():
        # returns the status of each component as well as the handle of object used to talk to component
    
        # Flight Computer
        fc_status, fc = fc_health_test() # what to do if there is an error?
    
        # Power Subsystem
        pwr_status, pwr = pwr_health_test()
    
        # ADCNS
        gyro_status, gyro = gyro_health_test()
        cam1_status, cam1 = cam_health_test(1)
        cam2_status, cam2 = cam_health_test(2)
        cam3_status, cam3 = cam_health_test(3)
    
        # Propulsion
        prop_status, prop = prop_health_test()
        electrolyzer_status, electrolyzer = electrolyzer_health_test()
    
        # dict {component_name (string) : component_status (string)}
        status = {'fc':fc_status, 'pwr':pwr_status, 'gyro':gyro_status, 'cam1':cam1_status, 'cam2':cam2_status, 'cam3':cam3_status, 'prop':prop_status, 'electrolyzer':electrolyzer_status}
        # component name is a short ID, like 'gyro', or 'cam1'
        # dict {component_name (string) : component_handle (object)}
        handles = {'fc':fc, 'pwr':pwr, 'gyro':gyro, 'cam1':cam1, 'cam2':cam2, 'cam3':cam3, 'prop':prop, 'electrolyzer':electrolyzer}
        # if component doesn't initialize properly, object handle is None
        return status, handles 
    
    def getCPUTemp():
        res = os.popen('vcgencmd measure_temp').readline()
        rev = (res.replace("temp=","").replace("'C\n",""))
        temp = float(rev)
        return temp
    
    def cam_health_test():
        # returns camera status (for all cameras) and handle of object(s) used to communicate with them (picamera handles?)
        # enclose most functions in try-catch blocks; if something goes wrong, the effects are isolated
        
        return
    
    def gyro_health_test():
        #Fill later
        return
    
    def propulsion_health_test():
        #Fill later
        return
    
    def electrolyzer_health_test():
        # Fill later
        return
    
    def HS_process_cmd_thread():
        result = 1 #current status OK
        bytesCopied = 0
        
        while(gvHS_commandQ != None and gvHS_command != None):
            bytesCopied = gvHS_commandQ.get() #Should we have timeout?
    ...
    def HS_execute_cmd():
        ...
        if(gvHS_commandQ == None):
            #Report false command execution
            result = 0
        else:
            CR_ExecuteCmd(gvHS_commandQ)
            #report true command execution
        
        return
    
    def HS_receive_cmd(commandPtr):
        result = 1; #current status ok
        result = gvHS_commandQ.put(commandPtr)
        return
